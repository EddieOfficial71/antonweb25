...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function openSearchPopupWindow() {
        ...
        try {
            const doc = popup.document;
            doc.open();
            doc.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>about:blank</title><style>body{font-family:Arial,Helvetica,sans-serif;margin:0;padding:18px;background:#f6f7fb} .container{max-width:920px;margin:24px auto;background:#fff;padding:18px;border-radius:8px;box-shadow:0 6px 20px rgba(0,0,0,0.08)} input{width:100%;padding:10px 12px;border:1px solid #ccc;border-radius:6px;font-size:16px} button{margin-top:10px;padding:10px 14px;border-radius:6px;background:#0b66c3;color:#fff;border:none;cursor:pointer} .small{font-size:13px;color:#666;margin-top:8px}</style></head><body><div class="container"><h2 style="margin-top:0">Search (privacy-friendly)</h2><form id="gform"><input id="ginput" type="search" placeholder="Type your search and press Enter" autocomplete="off" /><div style="display:flex;gap:8px;margin-top:10px"><button id="gsearch" type="submit">Search</button><button id="gclose" type="button" style="background:#6c757d">Close</button></div><p class="small">Search results will open in this window.</p></form></div><script> (function(){ var form=document.getElementById('gform'); var input=document.getElementById('ginput'); var btn=document.getElementById('gsearch'); var closeBtn=document.getElementById('gclose'); form.addEventListener('submit',function(e){ e.preventDefault(); var q=input.value.trim(); if(!q) return; var url='https://duckduckgo.com/?q='+encodeURIComponent(q); var win = window.open(url, '_blank', 'noopener,noreferrer'); try{ if(win) win.opener = null; }catch(e){} if(!win) window.location.href = url; }); closeBtn.addEventListener('click',function(){ try{ window.close(); }catch(e){} }); input.focus(); })(); <\/script></body></html>`);
            doc.close();
            try { popup.focus(); } catch (e) {}
            return true;
        } catch (err) {
    ...
    function openGoogleSearchFor(targetUrl) {
        try {
            ...
            const win = window.open(ddgUrl, '_blank', 'noopener,noreferrer');
            try { if (win) win.opener = null; } catch (e) {}
            if (!win) window.location.href = ddgUrl;
        } catch (err) {
